    ...
    .directive('googleChart', ['$timeout', '$window', '$rootScope', 'googleChartApiPromise', function ($timeout, $window, $rootScope, googleChartApiPromise) {
      return {
        ...
        link: function ($scope, $elm, $attrs) {
          ...
          // ASD Software Design LLC: disabled watch on all data. don't need it.
            ...
            if (typeof($scope.chart.formatters[formatType]) !== 'undefined') {
              ...
              if (!angular.equals($scope.chart.formatters[formatType], $scope.oldChartFormatters[formatType])) {
                ...
                if (formatType === 'color') {
                  ...
                  angular.forEach($scope.chart.formatters[formatType], function(formattersformatType) {

                    var colorFormat = new formatClass();

                    angular.forEach(formattersformatType.formats, function(data){
                    ...
                    $scope.formatters[formattersformatType].push(colorFormat);
                  });

                } else {
                  angular.forEach($scope.chart.formatters[formatType], function(formattersformatType){
                    $scope.formatters[formatType].push(new formatClass(formattersformatType));
                  });
                }
              }

              angular.forEach($scope.chart.formatters[formatType], function(formattersformatType){
                if (formattersformatType.column.columnNum < dataTable.getNumberOfColumns()) {
                  formattersformatType.format(dataTable, formattersformatType.columnNum);
                }
              });
